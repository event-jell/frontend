  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-5 mx-4">
        <h2 className="text-lg font-bold text-slate-800 mb-5">Add Guest</h2>
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-medium text-slate-600 mb-1 block">Group</label>
            ...
            <div>
              <label className="text-xs font-medium text-slate-600 mb-1 block">+1 Guests</label>
          ...
          <div>
            <label className="text-xs font-medium text-slate-600 mb-1 block">Dietary Requirements</label>
          ...
          <div>
            <label className="text-xs font-medium text-slate-600 mb-1 block">RSVP Status</label>
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="bg-white border-b border-slate-100 px-4 py-4 sm:px-8 sm:py-5 flex-shrink-0">
        <p className="text-sm text-slate-500 mb-0.5">Manage attendees</p>
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Guests</h1>
            {ticketId && (
              <div className="mt-1 flex items-center gap-2">
                <span className="text-xs bg-[#F5E6D3] text-[#7A1F1F] px-2 py-0.5 rounded-full font-medium">Filtered by ticket</span>
                <button onClick={() => setSearchParams({})} className="text-xs text-slate-500 hover:text-slate-700 hover:underline">Clear</button>
              </div>
            )}
          </div>
              ...
              {addMenuOpen && (
                <div className="absolute right-0 top-full mt-1 w-44 bg-white border border-slate-200 rounded-xl shadow-lg z-10 py-1" onMouseLeave={() => setAddMenuOpen(false)}>
                  <button onClick={() => { setShowAdd(true); setAddMenuOpen(false); }}
                    ...
                    <Plus size={14} /> Single guest
                  </button>
                  <button onClick={() => { setShowImport(true); setAddMenuOpen(false); }}
                    className="flex items-center gap-2 w-full px-4 py-2 text-sm text-slate-700 hover:bg-slate-50">
                    <Upload size={14} /> Import CSV / paste
                  </button>
                </div>
              )}
            </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 sm:px-8 sm:py-6 max-w-[1200px] mx-auto w-full">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
          {([
            { label: 'Total Guests', value: counts.total,     color: '#7A1F1F' },
            { label: 'Confirmed',    value: counts.confirmed, color: '#10B981' },
            { label: 'Pending',      value: counts.pending,   color: '#F59E0B' },
            { label: 'Declined',     value: counts.declined,  color: '#EF4444' },
            { label: 'Checked In',   value: counts.checkedIn, color: '#7A1F1F' },
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-4">
          <div className="relative flex-1 max-w-sm">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name, email, group…"
        ...
        {someSelected && (
          <div className="flex items-center gap-3 mb-3 px-4 py-2.5 bg-[#7A1F1F]/5 border border-[#7A1F1F]/20 rounded-xl">
            <span className="text-sm font-semibold text-[#7A1F1F]">{selected.size} selected</span>
            ...
            <button onClick={() => bulkCheckIn(true)}
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold bg-white border border-slate-200 rounded-lg hover:bg-slate-50 shadow-sm">
              <UserCheck size={13} /> Check in all
            </button>
        ...
        <div className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">RSVP</th>
              ...
              <tbody>
                {isLoading ? (
                  <tr><td colSpan={8} className="px-5 py-12 text-center text-slate-400">Loading guests…</td></tr>
                ) : filtered.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="px-5 py-16 text-center">
                      <div className="text-slate-400">
                        <p className="font-medium">No guests yet</p>
                        <p className="text-xs mt-1">Add a guest manually or import a CSV to get started</p>
          ...
          {filtered.length > 0 && (
            <div className="px-5 py-3 border-t border-slate-100 text-xs text-slate-400 flex items-center justify-between">
              <span>Showing {filtered.length} of {ticketFiltered.length} guests</span>
      ...
      {showCopied && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-slate-800 text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-3 z-50">
          ...
          <span className="font-medium text-sm">RSVP link copied!</span>
        </div>
      )}
    </div>
  );
