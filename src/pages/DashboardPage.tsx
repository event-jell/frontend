...
export default function DashboardPage() {
  ...
  return (
    <>
      <SEO title="Dashboard — EventJelly" description="Overview of your events, vendor services, and wallet" />
      ...
      <div className="min-h-full bg-gradient-to-br from-slate-50 via-white to-amber-50/20 p-4 sm:p-8 space-y-6 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white border border-slate-100 rounded-3xl p-6 sm:p-7 shadow-xs">
          ...
          <div className="flex items-center gap-2.5 flex-wrap">
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2.5 bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 text-xs font-bold rounded-xl shadow-2xs transition-all active:scale-98"
            >
              <Store size={15} className="text-[#7A1F1F]" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <div className="bg-gradient-to-br from-[#7A1F1F] via-[#661919] to-[#4A1212] text-white rounded-3xl p-6 shadow-xl shadow-[#7A1F1F]/20 relative overflow-hidden flex flex-col justify-between space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <button
                  ...
                  className="text-[11px] font-bold text-white/80 hover:text-white flex items-center gap-1 hover:underline"
                >
              ...
              <div className="flex items-center gap-4 mt-3 text-xs text-white/80">
                <div>
                  <span className="text-white/50 block text-[10px]">Total Earned</span>
                  <span className="font-bold">{formatCurrency(metrics?.total_earned || 0, localCurrency)}</span>
                </div>
                <div className="border-l border-white/20 pl-4">
                  <span className="text-white/50 block text-[10px]">Withdrawn</span>
            ...
            <div className="grid grid-cols-2 gap-2 pt-2 border-t border-white/15">
              <button
                ...
                className="py-2 px-3 rounded-xl bg-white text-[#7A1F1F] text-xs font-bold hover:bg-amber-50 flex items-center justify-center gap-1.5 shadow-sm transition-all active:scale-98"
              >
              ...
              <button
                ...
                className="py-2 px-3 rounded-xl bg-white/15 hover:bg-white/25 text-white text-xs font-bold flex items-center justify-center gap-1.5 transition-all active:scale-98"
              >
                <Plus size={14} />
                <span>Add Funds</span>
              </button>
          ...
          <div className="bg-white border border-slate-200/80 rounded-3xl p-6 shadow-xs flex flex-col justify-between space-y-4">
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <button
                  ...
                  className="text-[11px] font-bold text-[#7A1F1F] hover:underline flex items-center gap-1"
                >
            ...
            <button
              onClick={() => navigate('/vendor/listings/new')}
              className="w-full py-2.5 px-3 rounded-xl bg-[#FAF0E8] hover:bg-[#f3dfce] text-[#7A1F1F] text-xs font-bold flex items-center justify-center gap-1.5 border border-[#7A1F1F]/20 transition-colors"
            >
              <Plus size={14} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white border border-slate-100 rounded-3xl p-6 shadow-xs space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-base font-bold text-slate-900">Your Vendor Listings</h2>
                <p className="text-xs text-slate-500">Services currently discoverable by event planners</p>
              </div>
              <button
                onClick={() => navigate('/vendor/listings')}
            ...
            {(!summary?.vendor_listings || summary.vendor_listings.length === 0) ? (
              <div className="bg-[#FAF7F2] border border-dashed border-amber-200 rounded-2xl p-8 text-center space-y-3">
                <div className="w-12 h-12 rounded-2xl bg-[#FAF0E8] text-[#7A1F1F] flex items-center justify-center mx-auto">
                  <Store size={24} />
                </div>
                <div>
                  <h3 className="text-sm font-bold text-slate-900">No vendor listings yet</h3>
                ...
                <button
                  onClick={() => navigate('/vendor/listings/new')}
                  className="inline-flex items-center gap-1.5 px-4 py-2.5 bg-[#7A1F1F] text-white text-xs font-bold rounded-xl shadow-sm hover:bg-[#661919] transition-all"
                >
                  <Plus size={14} />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                {summary.vendor_listings.map((l) => (
                  <div
                    key={l._id}
                    onClick={() => navigate('/vendor/listings')}
                    className="group p-4 bg-slate-50 hover:bg-amber-50/40 border border-slate-200/80 rounded-2xl cursor-pointer transition-all hover:shadow-xs space-y-3"
                  >
                    <div className="flex items-start justify-between gap-2">
                      <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-[#FAF0E8] text-[#7A1F1F] border border-[#7A1F1F]/20">
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-slate-900 group-hover:text-[#7A1F1F] transition-colors line-clamp-1">
                        {l.title}
                      </h4>
          ...
          <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-xs space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500">Inflows and disbursements</p>
              </div>
              <button
                onClick={() => navigate('/wallet')}
                className="text-xs font-bold text-[#7A1F1F] hover:underline"
              ...
              {summary.recent_activities.map((act) => (
                <div key={act.id} className="py-3 flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="text-xs font-bold text-slate-800 block">{act.title}</span>
