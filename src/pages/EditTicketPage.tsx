...
import { ArrowLeft, Sparkles, Calendar, Users, DollarSign, Info, Minus, Plus, CheckCircle2 } from 'lucide-react';
import { useTicket, useUpdateTicket } from '../hooks/useTickets';
import { useEvent } from '../hooks/useEvents';
import { useLocale } from '../hooks/useLocale';
import SEO from '../components/SEO';
import DatePicker from '../components/DatePicker';
import { SUPPORTED_CURRENCIES, getCurrencySymbol, formatCurrency } from '../utils/formatters';
...
export default function EditTicketPage() {
  ...
  useEffect(() => {
    if (ticket) {
      setForm({
        ...
        currency: ticket.currency || event?.currency || localCurrency,
        total: ticket.total || 100,
        status: (ticket.status as 'active' | 'sold_out' | 'paused') || 'active',
        saleStart: ticket.saleStart ? ticket.saleStart.split('T')[0] : '',
        saleEnd: ticket.saleEnd ? ticket.saleEnd.split('T')[0] : '',
      });
    }
  }, [ticket, event?.currency, localCurrency]);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name.trim() || !ticketId) return;

    updateTicket.mutate(
      { id: ticketId, data: { ...form, eventId: id } },
      {
        onSuccess: () => {
          setIsSaved(true);
          toast.success('Ticket updated successfully!');
          setTimeout(() => {
            navigate(`/events/${id}/ticketing/${ticketId}`);
          }, 600);
        },
      }
    );
    ...
    return (
      <div className="flex-1 overflow-y-auto bg-[#F8FAFC] flex items-center justify-center p-12">
        <div className="text-center space-y-4">
          <h2 className="text-xl font-bold text-slate-800">Ticket Type Not Found</h2>
          <button
            onClick={() => navigate(`/events/${id}/ticketing`)}
            className="px-5 py-2.5 bg-[#7A1F1F] text-white font-bold text-xs rounded-xl"
          >
            Back to Ticketing
          </button>
        </div>
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-[#F8FAFC]">
      <SEO title={`Edit ${ticket.name || 'Ticket Type'}`} />
      <div className="max-w-5xl mx-auto px-4 py-8 sm:px-6 sm:py-12">
        {/* Back Button */}
        <button
          onClick={() => navigate(`/events/${id}/ticketing/${ticketId}`)}
        ...
        {/* Title */}
        <div className="mb-8">
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">Edit Ticket Type</h1>
          <p className="text-sm text-slate-500 mt-1 font-medium">
            Update pricing, description, availability, and access settings for <strong>{ticket.name}</strong>.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <form onSubmit={handleSubmit} className="lg:col-span-7 bg-white rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/40 p-8 space-y-6">
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 bg-slate-50/50 border border-slate-200 rounded-xl text-slate-800 text-sm font-medium focus:outline-none focus:bg-white focus:border-[#7A1F1F] focus:ring-4 focus:ring-[#7A1F1F]/5 transition-all placeholder:text-slate-400"
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-4 py-3 bg-slate-50/50 border border-slate-200 rounded-xl text-slate-800 text-sm font-medium focus:outline-none focus:bg-white focus:border-[#7A1F1F] focus:ring-4 focus:ring-[#7A1F1F]/5 transition-all placeholder:text-slate-400 resize-none"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-12 gap-4">
                ...
                <div className="sm:col-span-4">
                  ...
                  <div className="flex items-center bg-slate-50/50 border border-slate-200 rounded-xl focus-within:bg-white focus-within:border-[#7A1F1F] focus-within:ring-4 focus-within:ring-[#7A1F1F]/5 transition-all overflow-hidden">
                    <button
                      type="button"
                      onClick={() => setForm(f => ({ ...f, total: Math.max(ticket.sold || 1, f.total - 10) }))}
                      className="px-2.5 py-3 text-slate-400 hover:text-slate-750 hover:bg-slate-100/80 active:bg-slate-200/50 transition-colors border-r border-slate-200 select-none font-bold text-base shrink-0"
                    >
                      <Minus size={13} />
                    </button>
                    <input
                      ...
                      min={ticket.sold || 1}
                      value={form.total}
                      onChange={e => setForm(f => ({ ...f, total: Math.max(ticket.sold || 1, Number(e.target.value)) }))}
                      className="w-full bg-transparent px-2 py-2 text-center text-slate-800 text-sm font-bold focus:outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                    />
                    <button
                      type="button"
              ...
              {/* Status Selector */}
              <div>
                <label className="text-sm font-bold text-slate-700 mb-1.5 block">Ticket Status</label>
                <div className="grid grid-cols-3 gap-3">
                  {[
                    { id: 'active', label: 'Active', desc: 'Available for registration' },
                    { id: 'sold_out', label: 'Sold Out', desc: 'No new registrations' },
                    { id: 'paused', label: 'Paused', desc: 'Temporarily unavailable' },
                  ].map(st => (
                    <button
                      key={st.id}
                      type="button"
                      onClick={() => setForm(f => ({ ...f, status: st.id as any }))}
                      className={'p-3 rounded-xl border text-left transition-all ' + (
                        form.status === st.id
                          ? 'border-[#7A1F1F] bg-[#FAF0E8] text-[#7A1F1F] ring-2 ring-[#7A1F1F]/20'
                          : 'border-slate-200 bg-slate-50/50 text-slate-600 hover:bg-slate-100/60'
                      )}
                    >
                      <div className="font-bold text-xs capitalize">{st.label}</div>
            ...
            <div className="pt-6 border-t border-slate-100 flex items-center justify-end gap-3">
              ...
              <button
                type="submit"
                disabled={!form.name.trim() || updateTicket.isPending}
          ...
          <div className="lg:col-span-5 space-y-4">
            ...
            <div className="relative bg-white border border-slate-200 rounded-3xl shadow-xl shadow-slate-200/40 overflow-hidden select-none">
              ...
              <div className="p-6 pt-2 space-y-4">
                <div className="grid grid-cols-2 gap-4 text-xs">
                  <div>
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Capacity & Sales</span>
                    <p className="font-bold text-slate-700 mt-0.5">{ticket.sold || 0} / {form.total} claimed</p>
                  </div>
                  <div>
            ...
            <div className="bg-[#E0F2FE]/40 border border-[#BAE6FD] rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-xs text-sky-800 leading-relaxed">
                Changes saved here take effect immediately for all upcoming registrations and update the live QR scan passes.
              </p>
            </div>
          </div>
