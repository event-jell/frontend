  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-5 mx-4">
        <h2 className="text-lg font-bold text-slate-800 mb-5">{t('ticketing.create_title')}</h2>
        ...
        <div className="flex items-center justify-end gap-2 mt-6">
          ...
          >
            {isPending ? t('common.creating') : t('ticketing.create_title')}
          </button>
        </div>
      </div>
    </div>
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      {showCopied && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-slate-800 text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-3 z-50 animate-in fade-in slide-in-from-bottom-4 duration-300">
          ...
          <span className="font-medium text-sm">{t('ticketing.copied')}</span>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 sm:px-8 sm:py-6 max-w-[1200px] mx-auto w-full">
        ...
        {/* Ticket cards */}
        {isLoading ? (
          <div className="flex items-center justify-center py-20 text-slate-400">{t('ticketing.loading')}</div>
        ) : tickets.length === 0 ? (
                      ...
                      {openMenu === ticket._id && (
                        <div className="absolute right-0 top-8 bg-white border border-slate-100 rounded-xl shadow-lg z-10 py-1 w-36">
                          <button onClick={(e) => { e.stopPropagation(); navigate(`/events/${id}/ticketing/${ticket._id}`); setOpenMenu(null); }}
                            className="w-full text-left px-3 py-2 text-sm text-slate-600 hover:bg-slate-50">{t('ticketing.view_guests')}</button>
                          <button onClick={(e) => {
                            e.stopPropagation();
                            handleCopyLink();
                          }}
                            className="w-full text-left px-3 py-2 text-sm text-slate-600 hover:bg-slate-50">{t('ticketing.copy_link')}</button>
                          {ticket.status !== 'paused' && (
                            <button onClick={(e) => { e.stopPropagation(); updateTicket.mutate({ id: ticket._id, data: { status: 'paused' } }); setOpenMenu(null); }}
                              className="w-full text-left px-3 py-2 text-sm text-slate-600 hover:bg-slate-50">{t('ticketing.pause_sales')}</button>
                          )}
                          {ticket.status === 'paused' && (
                            <button onClick={(e) => { e.stopPropagation(); updateTicket.mutate({ id: ticket._id, data: { status: 'active' } }); setOpenMenu(null); }}
                              className="w-full text-left px-3 py-2 text-sm text-slate-600 hover:bg-slate-50">{t('ticketing.resume_sales')}</button>
                          )}
                          <button onClick={(e) => { e.stopPropagation(); if (confirm('Delete this ticket type?')) { deleteTicket.mutate(ticket._id); setOpenMenu(null); } }}
                            className="w-full text-left px-3 py-2 text-sm text-red-500 hover:bg-slate-50">{t('common.delete')}</button>
                        </div>
                      )}
                    </div>
                  </div>
