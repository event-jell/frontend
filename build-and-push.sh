...
docker build \
  ...
  --build-arg VITE_API_URL="${VITE_API_URL:-${API_URL:-}}" \
  --build-arg VITE_SOCKET_URL="${VITE_SOCKET_URL:-${SOCKET_URL:-}}" \
  --build-arg API_URL="${API_URL:-${VITE_API_URL:-}}" \
  --build-arg SOCKET_URL="${SOCKET_URL:-${VITE_SOCKET_URL:-}}" \
  --tag "${FULL_IMAGE}" \
  --tag "${IMAGE}:latest" \
  "${SCRIPT_DIR}"
